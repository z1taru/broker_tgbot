...
from keyboards.inline import get_questions_keyboard, get_back_keyboard
...
async def show_faq_answer(callback: CallbackQuery):
    ...
    try:
        ...
        # Если есть видео — отправляем видео с текстом в одном сообщении
        if faq.get("video_url"):
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(video_full_url) as video_resp:
                        if video_resp.status == 200:
                            ...
                            # Отправляем видео с caption (текстом)
                            ...
                            # Если не удалось загрузить видео, отправляем только текст
                ...
                # Если ошибка при отправке видео, отправляем только текст
                await callback.message.answer(
                    caption_text,
                    reply_markup=keyboard
                )
